 ...
 * engines (exposed as `platform.forecast`) bound to the SAME EventLog the platform
...
beforeEach(() => {
  ...
  storeDir = mkdtempSync(join(tmpdir(), "a9-cr-store-"));
});
...
describe("A9 composition-root wiring (CapabilityPlatform.a9)", () => {
  it("exposes the constructed A9 adapters + engines bound to the platform EventLog", () => {
    ...
    expect(platform.forecast.proposalEvents).toBeInstanceOf(ProposalEventsAdapter);
    expect(platform.forecast.measurementEvents).toBeInstanceOf(MeasurementEventsAdapter);
    expect(platform.forecast.enrichedProposals).toBeInstanceOf(EnrichedProposalsAdapter);
    expect(platform.forecast.forecasts).toBeInstanceOf(ForecastsAdapter);
    expect(platform.forecast.correlations).toBeInstanceOf(CorrelationsAdapter);
    expect(platform.forecast.forecastEngine).toBeInstanceOf(ForecastEngine);
    expect(platform.forecast.correlationEngine).toBeInstanceOf(CorrelationEngine);
  });

  it("generates a forecast from the platform EventLog and the adapter query surface reads it back", async () => {
    const eventLog = new EventLog(sessionDir);
    const platform = new CapabilityPlatform({ catalogDir, eventLog, storeDir });
    ...
    const forecasts = await platform.forecast.forecastEngine.forecast(NOW);
    ...
    // store dir).
    const store = new ForecastsStore(storeDir);
    await store.append(forecasts[0]!);
    const viaAdapter = await platform.forecast.forecasts.findByProposalId("prop-1");
  ...
  it("correlation engine is wired over the platform adapters (automatic correlation, no operator mutation)", async () => {
    ...
    const correlations = await platform.forecast.correlationEngine.correlate(NOW);
  ...
  it("derives REAL EnrichedProposal[] from .alix stores when none injected (evidence-completeness detector live)", async () => {
    ...
    try {
      ...
      const platform = new CapabilityPlatform({ catalogDir, eventLog, storeDir });
      const records = await platform.forecast.enrichedProposals.list();

...
describe("edge cases", () => {
  ...
  it("normal balanced events with baseline → zero anomalies", () => {
    // Every runtime.allowed has a approval.created on the same trace.
    // Timestamps are monotonically increasing in append order (no regression).
    const events = defineEvents([
      { eventId: "e1", timestamp: T, eventType: "approval.created", traceId: "t1", riskLevel: "low" },
      { eventId: "e2", timestamp: T1, eventType: "runtime.allowed", traceId: "t1", riskLevel: "low", metadata: { requiresHumanReview: true } },
      { eventId: "e3", timestamp: T2, eventType: "policy.evaluated", traceId: "t1", riskLevel: "low" },
    ]);
    // Normal baseline: same event types at similar ratios
    const baseline = defineEvents([
      { eventId: "b1", timestamp: T_EARLY, eventType: "approval.created", traceId: "t0", riskLevel: "low" },
      { eventId: "b2", timestamp: T_EARLY, eventType: "runtime.allowed", traceId: "t0", riskLevel: "low" },
      { eventId: "b3", timestamp: T_EARLY, eventType: "policy.evaluated", traceId: "t0", riskLevel: "medium" },
    ]);
...
describe("volume anomalies", () => {
  it("no baseline → zero volume anomalies", () => {
    const events = defineEvents([
      { eventId: "v1", eventType: "runtime.blocked" },
    ]);
    assert.equal(detectAnomalies(events).filter((a) => a.type.startsWith("volume_")).length, 0);
  });

  it("spike in runtime.blocked at 3× baseline → warning", () => {
    const events = defineEvents([
      { eventId: "s1", eventType: "runtime.blocked" },
      { eventId: "s2", eventType: "runtime.blocked" },
      { eventId: "s3", eventType: "runtime.blocked" },
      { eventId: "s4", eventType: "runtime.blocked" },
      { eventId: "s5", eventType: "runtime.blocked" },
      { eventId: "s6", eventType: "runtime.blocked" },
    ]);
    const baseline = defineEvents([
      { eventId: "b1", eventType: "runtime.blocked" },
      { eventId: "b2", eventType: "runtime.blocked" },
    ]);
  ...
  it("zero baseline + current 2 events → no anomaly", () => {
    const events = defineEvents([
      { eventId: "z1", eventType: "runtime.blocked" },
      { eventId: "z2", eventType: "runtime.blocked" },
    ]);
    const baseline = defineEvents([
      { eventId: "b1", eventType: "policy.evaluated" },
    ]);
    // Baseline has 0 runtime.blocked, current has 2 (< 3) → no anomaly
  ...
  it("zero baseline + current 5 events → volume_spike critical", () => {
    const events = defineEvents([
      { eventId: "z1", eventType: "runtime.blocked" },
      { eventId: "z2", eventType: "runtime.blocked" },
      { eventId: "z3", eventType: "runtime.blocked" },
      { eventId: "z4", eventType: "runtime.blocked" },
      { eventId: "z5", eventType: "runtime.blocked" },
    ]);
    const baseline = defineEvents([
      { eventId: "b1", eventType: "policy.evaluated" },
    ]);
  ...
  it("drop in approval.created below 0.25× baseline → warning", () => {
    const events = defineEvents([
      { eventId: "d1", eventType: "approval.created" },
    ]);
    const baseline = defineEvents([
      { eventId: "b1", eventType: "approval.created" },
      { eventId: "b2", eventType: "approval.created" },
      { eventId: "b3", eventType: "approval.created" },
      { eventId: "b4", eventType: "approval.created" },
      { eventId: "b5", eventType: "approval.created" },
    ]);
...
describe("risk anomalies", () => {
  it("no baseline → zero risk anomalies", () => {
    const events = defineEvents([
      { eventId: "r1", eventType: "runtime.allowed", riskLevel: "critical" },
    ]);
  ...
  it("risk shift with <5 decision events → zero risk anomalies (minimum sample)", () => {
    ...
    const baseline = defineEvents([
      { eventId: "b1", eventType: "runtime.allowed", riskLevel: "low" },
    ]);
  ...
  it("high critical ratio with ≥5 decision events → risk_shift", () => {
    ...
    const events = defineEvents([
      { eventId: "c1", eventType: "runtime.allowed", riskLevel: "critical" },
      { eventId: "c2", eventType: "runtime.allowed", riskLevel: "critical" },
      { eventId: "c3", eventType: "runtime.blocked", riskLevel: "critical" },
      { eventId: "c4", eventType: "runtime.requires_approval", riskLevel: "critical" },
      { eventId: "c5", eventType: "runtime.allowed", riskLevel: "low" },
    ]);
    const baseline = defineEvents([
      { eventId: "b1", eventType: "runtime.allowed", riskLevel: "low" },
      { eventId: "b2", eventType: "runtime.allowed", riskLevel: "low" },
      { eventId: "b3", eventType: "runtime.allowed", riskLevel: "low" },
      { eventId: "b4", eventType: "runtime.allowed", riskLevel: "low" },
      { eventId: "b5", eventType: "runtime.allowed", riskLevel: "medium" },
      { eventId: "b6", eventType: "runtime.allowed", riskLevel: "medium" },
      { eventId: "b7", eventType: "runtime.allowed", riskLevel: "medium" },
      { eventId: "b8", eventType: "runtime.allowed", riskLevel: "medium" },
      { eventId: "b9", eventType: "runtime.allowed", riskLevel: "critical" },
      { eventId: "b10", eventType: "runtime.allowed", riskLevel: "critical" },
    ]);
...
describe("sequence anomalies", () => {
  it("runtime.allowed without request on trace → approval_without_request", () => {
    const events = defineEvents([
      { eventId: "a1", eventType: "runtime.allowed", traceId: "t1", metadata: { requiresHumanReview: true } },
    ]);
    const seq = detectAnomalies(events).filter((a) => a.type === "approval_without_request");
    assert.equal(seq.length, 1);
  });

  it("runtime.allowed WITH preceding request on same trace → no anomaly", () => {
    const events = defineEvents([
      { eventId: "r1", eventType: "approval.created", traceId: "t1" },
      { eventId: "a1", eventType: "runtime.allowed", traceId: "t1", metadata: { requiresHumanReview: true } },
    ]);
    assert.equal(detectAnomalies(events).filter((a) => a.type === "approval_without_request").length, 0);
  });

  it("runtime.requires_approval without review context → escalation_without_review", () => {
    const events = defineEvents([
      { eventId: "e1", eventType: "runtime.requires_approval", traceId: "t1" },
    ]);
    assert.equal(detectAnomalies(events).filter((a) => a.type === "escalation_without_review").length, 1);
  });

  it("runtime.requires_approval WITH policy.evaluated on same trace → no anomaly", () => {
    const events = defineEvents([
      { eventId: "p1", eventType: "policy.evaluated", traceId: "t1" },
      { eventId: "e1", eventType: "runtime.requires_approval", traceId: "t1" },
    ]);
  ...
  it("terminal overridden + later allow on same trace+subject → terminal_mutation", () => {
    const events = defineEvents([
      { eventId: "t1", eventType: "override.applied", traceId: "t1", subjectId: "sig-001", timestamp: T },
      { eventId: "t2", eventType: "runtime.allowed", traceId: "t1", subjectId: "sig-001", timestamp: T1 },
    ]);
  ...
  it("terminal mutation with different subjectId → no anomaly", () => {
    const events = defineEvents([
      { eventId: "t1", eventType: "override.applied", traceId: "t1", subjectId: "sig-001", timestamp: T },
      { eventId: "t2", eventType: "runtime.allowed", traceId: "t1", subjectId: "sig-002", timestamp: T1 },
    ]);
...
describe("output structure", () => {
  ...
  it("anomalyId is deterministic (same input → same id)", () => {
    const events1 = defineEvents([
      { eventId: "fix-1", eventType: "runtime.allowed", traceId: "t-fix", metadata: { requiresHumanReview: true } },
    ]);

 ...
 *
 * Asserts that GovernanceProposalStore:
...
describe("GovernanceProposalStore — append-only governance ledger (CAP-9)", () => {
  let dir: string;
  let eventLog: EventLog;
  let store: GovernanceProposalStore;

  beforeEach(async () => {
    dir = mkdtempSync(join(tmpdir(), "cap9-store-"));
    eventLog = new EventLog(dir);
    await eventLog.init();
    store = new GovernanceProposalStore({ eventLog });
  });

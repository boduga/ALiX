...
function makeRepoMap(sourceFiles: string[] = [], deps: Record<string, string[]> = {}): RepoMapOutput {
  ...
  const dependencyGraph = {
    ...
    dependentsOf() { return []; },
    impactScore() { return 0; },
    transitiveDependenciesOf() { return []; },
    files: [] as string[],
  };
  return {
    sourceFiles,
...
test.describe("ContextPipeline feature regression", () => {
  ...
  test.it("includes symbol matches with score 80", async () => {
    ...
    const repoMap = {
      ...
      docsFiles: [],
      fileEntries,
      dependencyGraph: { _map: new Map(), dependenciesOf() { return []; }, dependentsOf() { return []; }, impactScore() { return 0; }, transitiveDependenciesOf() { return []; }, files: [] },

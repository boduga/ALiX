...
describe("AdaptationProposalStore", () => {
  ...
  // #704: list is stable across calls and reflects edits (cache invalidated
  // on save). Corrupt-file handling is covered by the existing suite.
  it("list is stable across calls and reflects edits", async () => {
    const base = {
      id: "prop-cache", createdAt: "2026-06-19T00:00:00Z", status: "pending" as const,
      action: "create_agent_card" as const, target: { kind: "agent_card" as const, id: "x" },
      payload: {}, sourceRecommendationType: "capability_gap",
      sourceConfidence: 0.9, evidenceFingerprints: [], reason: "one",
    };
    await store.save(base);
    const first = await store.list();
    const second = await store.list();
    expect(first).toEqual(second);
    expect(first[0]!.reason).toBe("one");

    // A real edit (via the store) invalidates the cache.
    await store.update("prop-cache", { reason: "three" });
    expect((await store.list())[0]!.reason).toBe("three");
  });
});

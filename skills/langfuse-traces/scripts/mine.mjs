 ...
 * --min-runs/--min-score here are a cheap prefilter (one fetch per trace
 * is the expensive part); the factory re-enforces the candidate bar
 * authoritatively on unique traceIds — overrides can differ, the factory
...
async function main() {
  ...
  for (const traceId of high) {
    ...
    if (!groups.has(key)) groups.set(key, { toolSequence: seq, traceIds: [], sessions: new Set() });
    const group = groups.get(key);
    group.traceIds.push(traceId);
    for (const o of data) {
      if (typeof o.sessionId === "string") group.sessions.add(o.sessionId);
    }
  }

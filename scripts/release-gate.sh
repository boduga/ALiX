...
echo "▸ npm pack --json..."
PACK_JSON="$(npm pack --json --pack-destination "$PROJECT_ROOT" 2>/dev/null)"
...
if [ -z "$TARBALL" ] || [ ! -f "$PROJECT_ROOT/$TARBALL" ]; then
  echo "  ❌ npm pack failed — no tarball produced"
  GATE_PASSED=false
fi
...
if [ -n "$TARBALL" ] && [ -f "$PROJECT_ROOT/$TARBALL" ]; then
  ...
  if npm install --prefix "$TMP_DIR" "$PROJECT_ROOT/$TARBALL" > /dev/null 2>&1; then

...
| D2 | **Cursor format is versioned internally.** `{ version: 1, seq }`. Future migration switches on `version`. `deserializeCursor` has exactly four failure modes: **malformed JSON**, **unsupported version**, **invalid payload**, **cursor position beyond the current EventLog head** — all throw `EventLogCursorError` (a dedicated exported class so callers can discriminate via `instanceof` against operational failures like a disk read or save rejection). `readSince` defensively re-validates and throws the same error class for a cursor whose `seq` exceeds the current head, so the invariant holds even if an invalid cursor bypasses the deserialize path. |
...
- **Missing / malformed / unknown-version / invalid payload / beyond-head position** on recovery — fall back to `beginningCursor()` (deterministic replay). All four `deserializeCursor` failure modes throw `EventLogCursorError`; the collector's `initializeCheckpoint` catches it (via `instanceof`) and resets. `readSince` also throws `EventLogCursorError` for a beyond-head cursor, so the invariant holds even if an invalid cursor bypasses the deserialize path — the collector's `sample` catches that case too and resets the checkpoint.
- **Operational failures** (disk read rejection on `load`, save rejection on `save`, any non-`EventLogCursorError` throw) — preserve the current checkpoint + cache and retry on the next sample. The two error classes are discriminated by `instanceof EventLogCursorError` so the invalid-cursor path always resets (full replay) while the operational path never blanks the dashboard.
- "Foreign" cursor handling: D1 persists no owner token, so `deserializeCursor` cannot tell whether the bytes came from a sibling log instance at load time. The restored cursor is owned by **this** EventLog and looks valid at load; rejection happens at the next `readSince` because the cursor's seq no longer matches this log's head — and that rejection is an `EventLogCursorError` (beyond-head) which forces a replay from `beginningCursor()` on the **following** sample.

...
- `renderBottomAnchoredSlice(opts: { canvas: TerminalCanvas; allLines: ScrollbackLine[]; top: number; bottomRow: number; offset: number; columns: number; kindStyles: Record<ScrollbackLineKind, (line: ScrollbackLine, rowY: number) => void> }): { firstRow: number; lastRow: number }`
  ...
  - **Clamping model: snap-to-start** (matches the reference implementation in the implementation plan). When `offset < 0`, `windowStart` clamps to `0` and the window runs for the full `scrollbackRows` from there (e.g. `offset=-5, scrollbackRows=16` renders `[0, 16)`, not `[0, 11)`). Endpoint-clamping (window also shrinks) is not implemented because it produces surprising behavior for the common case where `offset` arrives as a clamped result of upstream math and the user expects a full-viewport render. **Negative `offset` is unreachable from current callers** (pinned path returns `max(0, ...)`; unpinned path is bounded by `scrollOffset >= 0` enforced by `onActivate`/`End`/`clear`/`scroll-up`).

...
| `alix skills available`, `install <name> [--from <path\|url>]`, `install --list`, `marketplace list`, `add <name> <url>`, `remove <name>` | Marketplace skill installer: list skills across registered marketplaces (defaults: anthropics/skills, langfuse/skills), install by name or from an explicit path/https URL, and register/unregister marketplaces. `install <name> --from <dir>` installs the whole skill package — SKILL.md plus supporting files (scripts/, assets/, LICENSE) — copied from the source directory wholesale (minus vendored/tooling dirs like .git and node_modules); a single `.md` file or https URL still installs only SKILL.md. A repo-root dir with nested skills (e.g. a clone of anthropics/skills) resolves `skills/<name>/` when a name is given, or the unique nested skill otherwise; a name that matches nothing errors rather than silently installing a different skill. | `alix skills install brand-guidelines` | Handler suites under `tests/cli/commands/skills/` (`install.test.ts`, `marketplace.test.ts`, `run-skills.test.ts`); package-dir copy and exclusion behavior covered in `install.test.ts`; network-dependent listing/install is Manual/external. |

...
| `alix skills available`, `install <name> [--from <path\|url>]`, `install --list`, `marketplace list`, `add <name> <url>`, `remove <name>` | Marketplace skill installer: list skills across registered marketplaces (defaults: anthropics/skills, langfuse/skills), install by name or from an explicit path/https URL, and register/unregister marketplaces. | `alix skills install brand-guidelines` | Handler suites under `tests/cli/commands/skills/` (`install.test.ts`, `marketplace.test.ts`, `run-skills.test.ts`); network-dependent listing/install is Manual/external. |

...
- Coordination workers keep `worker.id` as their unique roster execution identity. Their canonical `agent.*` lifecycle events are stamped with the parent runtime session (not the coordination manager's internal id), so session-scoped roster projections observe them. Optional `coordinationRunId`, planner-assigned agent label, and task label flow as presentation metadata; the TUI never reads `CoordinationStore` as a second truth source.

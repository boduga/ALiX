...
- While the agent drawer is explicitly open, its selected agent filters only transcript items carrying a different authoritative `agentId`; uncorrelated historical items remain visible. Closing the drawer removes the presentation filter without changing evidence or selection history.

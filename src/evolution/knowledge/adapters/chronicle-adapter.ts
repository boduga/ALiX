...
import { readTextFileOrNull, type AdapterResult } from "./shared.js";
  ...
  async read(): Promise<AdapterResult> {
    try {
      ...
      if (!existsSync(chronicleDir)) {
        return { artifacts: [], status: { status: "unavailable", store: "chronicle" } };
      }
      ...
      return { artifacts, status: { status: "available", store: "chronicle" } };
    } catch (err) {
      return {
        artifacts: [],
        status: { status: "unavailable", store: "chronicle", reason: (err as Error).message ?? String(err) },
      };
    }
  }
}

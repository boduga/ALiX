 ...
 *     `A9ForecastContent` / `A9CorrelationContent`, which structurally lack
...
export function canonicalizeForecast(forecast: A9ForecastContent): string {
  return canonicalStringify(forecast);
}

/** Content-addressed forecast id: SHA-256 hex of the canonical content. */
export function forecastIdFor(forecast: A9ForecastContent): ForecastId {
...
export function canonicalizeCorrelation(correlation: A9CorrelationContent): string {
  return canonicalStringify(correlation);
}

/** Content-addressed correlation id: SHA-256 hex of the canonical content. */
export function correlationIdFor(correlation: A9CorrelationContent): CorrelationId {

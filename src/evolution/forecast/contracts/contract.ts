 ...
 *
 *   - Forecast layer is correlation-free. `Forecast` / `Correlation` MUST
 ...
 *
 * @module evolution/forecast/contracts
 */
...
/** Content-addressed identity of an Forecast (SHA-256 hex of canonical content). */
export type ForecastId = string;

/** Content-addressed identity of an Correlation (SHA-256 hex of canonical content). */
...
// ---------------------------------------------------------------------------
// Forecast — the forecast artifact
// ---------------------------------------------------------------------------
 ...
 * is a separate artifact (`Correlation`) built by a later slice.
 */
...
/** Canonical forecast content — `Forecast` without its content-addressed id.
...
export type ForecastContent = Omit<Forecast, "forecastId">;

// ---------------------------------------------------------------------------
// Correlation — forecast ⇄ measurement linkage (later slice; contract now)
// ---------------------------------------------------------------------------
...
/** Canonical correlation content — `Correlation` without its content-addressed id. */
export type CorrelationContent = Omit<Correlation, "correlationId">;
...
export interface ForecastAdapter<T> {

 ...
 *
 * @module evolution/forecast/adapters/proposal-events-adapter
 */

import type { EventLog } from "../../../events/event-log.js";
import type { ForecastAdapter, ProposalEventRecord } from "../contracts/contract.js";
import { readCandidateTargetId } from "../bridge-target.js";

/** Locked governance proposal event prefix (CAP-9 ruling #1/#2) — filter key. */
export const PROPOSAL_EVENT_PREFIX = "capability.governance.proposal.";
...
export const GOVERNANCE_NAMESPACE_PREFIX = "capability.governance.";

export class ProposalEventsAdapter implements ForecastAdapter<ProposalEventRecord> {
  ...
  async list(): Promise<ReadonlyArray<ProposalEventRecord>> {
    ...
    return all
      .filter((e) => e.type.startsWith(PROPOSAL_EVENT_PREFIX))

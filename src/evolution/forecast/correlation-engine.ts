 ...
 * independent `Correlation` per qualifying pair.
 *
 ...
 *   Step 5 — Emit ONE independent `Correlation` per (forecastId, measurementId)
 ...
 *
 * @module evolution/forecast/correlation-engine
 */
...
export interface CorrelationEngineDependencies {
  ...
  readonly proposalEvents: ForecastAdapter<ProposalEventRecord>;
  /** Canonical measurement events (Q8: capability-targeted, no proposal linkage). */
  readonly measurements: ForecastAdapter<CapabilityMeasurementRecord>;
}
...
export class CorrelationEngine {
   ...
   * @returns one independent Correlation per qualifying (forecastId, measurementId)
   *          pair, sorted by forecastId then measurementId
   */
  async correlate(timestamp: string): Promise<ReadonlyArray<Correlation>> {
  ...
  async correlateMeasurement(
    ...
    timestamp: string,
  ): Promise<ReadonlyArray<Correlation>> {

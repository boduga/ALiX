 ...
 *
 * Append-only JSONL persistence for Correlation artifacts.
 *
 ...
 *
 * @module evolution/forecast/correlations-store
 */
...
export class CorrelationsStore extends JsonlStore<Correlation, CorrelationId> {
   ...
   */
  async findByForecastId(forecastId: string): Promise<ReadonlyArray<Correlation>> {
   ...
   * Index operation over the single JSONL structure — NOT a new store.
   */
  async findByMeasurementId(measurementId: string): Promise<ReadonlyArray<Correlation>> {
    const all = await this.list();
 ...
 * false collision).
 */
export function correlationContentOf(correlation: Correlation): string {

...
/**
 * CAP-9 Task 4 — GovernanceProposalStore (append-only governance ledger wrapper).
 *
 * Lifecycle vocabulary (#716): submitted → approved|rejected →
 * executed|execution_failed EVENTS (event-sourced, not a status field).
 * This is a DIFFERENT lifecycle from the adaptation CRUD store
 * (AdaptationProposalStore record statuses: pending → approved|rejected
 * → applied|failed). The two never convert. Rough correspondence for
 * readers: pending≈submitted, applied≈executed, failed≈execution_failed.
 *
 ...
 *   - GovernanceProposalStore carries no live catalog/registry references; it only
...
export class GovernanceProposalStore {

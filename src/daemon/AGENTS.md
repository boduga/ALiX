...
- `scheduled-task-service.ts` — not here; the daemon constructs `ScheduledTaskService` (`../schedule/`) on listen and ticks it every 60s (materialize human-approved `schedule.propose` approvals → enqueue due jobs on the task queue; cleared on SIGTERM). Logic lives in `src/schedule/`.

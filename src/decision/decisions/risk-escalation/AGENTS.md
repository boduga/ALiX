...
- `local-baseline.ts` — `classifyRiskLocally`: read-only capabilities cannot mutate (low); destructive markers high; mutation markers or anything unknown medium.
- `corpus.ts` — labeled action fixtures + adversarial-instruction fixtures.
- `jev-mapping.ts` — `toJevRiskRequest` / `fromJevRiskResponse`; unknown tiers are malformed.

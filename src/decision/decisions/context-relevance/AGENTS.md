...
- `thresholds.ts` — versioned, engine-specific profiles + `thresholdProfileForEngine` (throwing), `tryThresholdProfileForEngine` (for journaling), `resolveProfileForEngine` (configured profile when it matches the answering engine, else that engine's own).

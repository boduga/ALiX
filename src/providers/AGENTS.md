...
| `free-model-resolver.ts` | Capability-aware selection of a concrete free model from the catalog; `resolveModelBySelectionPolicy` maps a `ModelSelectionPolicy` (cost/capabilities/minContext) onto the shared capability vocabulary |
...
- **Policy-driven selection (durable):** `ModelConfig.selection` (`ModelSelectionPolicy`: `provider`/`cost`/`capabilities`/`minContext`) expresses requirements; discovery supplies the model id. `buildRoutingAdapter` resolves `selection` to a concrete catalog model (via `resolveModelBySelectionPolicy`, honoring the bounded-lifetime restriction registry) as the primary. `cost: free` is served by the free-filtered catalog; `paid`/non-openrouter are not (fall back to the explicit `name` or fail clearly). `isValidModelConfig` accepts `provider && (name || selection)`. Design rule: the current free-model list is never hard-coded.

...
| `free-model-resolver.ts` | Capability-aware selection of a concrete free model from the catalog; `resolveModelBySelectionPolicy` maps a `ModelSelectionPolicy` (cost/capabilities/minContext) onto the shared capability vocabulary; `resolveModelSelectionId` is the async discovery seam (catalog fetch + restriction registry) shared by `createProvider` and `buildRoutingAdapter` |
...
- **Policy-driven selection (durable):** `ModelConfig.selection` (`ModelSelectionPolicy`: `provider`/`cost`/`capabilities`/`minContext`) expresses requirements; discovery supplies the model id. The single discovery seam is `resolveModelSelectionId` (`free-model-resolver.ts`), used by `createProvider` (the registry choke point — every consumer with a `selection` resolves here) and `buildRoutingAdapter` (which pre-resolves so its fallback chain/labels see a concrete `name`). Effective model = `name ?? model`; non-null `selection` overrides both unless unsatisfiable, in which case an explicit `name` is kept or a clear error is thrown. `cost: free` is served by the free-filtered catalog; `paid`/non-openrouter are not. `isValidModelConfig` accepts `provider && (name || selection)`. This also fixes the historical `.model`-undefined bug: call sites that pass a full `ModelConfig` (`plan`, runtime routes) now deliver `.name`. Design rule: the current free-model list is never hard-coded.

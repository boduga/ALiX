...
- **Free route:** `openrouter/free` (and any `:free`) performs current-model discovery, free-only filtering, restricted-id exclusion, and deterministic selection with NO hard-coded model IDs. It uses the REQUEST-DERIVED `resolveConcreteFreeModel(requirements)` (forced `needsTools:true` for the agent tab + request vision/structured-output) — NOT policy `selectModelFromDiscovery` — and since discovery returns the full catalog, filters `costPerMTokIn === 0` first, excluding bound-lifetime access-restricted ids + already-tried ids. Never globally cached. `alix models free` lists only `costPerMTokIn === 0` models.

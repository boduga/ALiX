...
- **Self-awareness is first-class (durable).** The system prompt must state the
  ...
  questions at `state.query`. A deterministic guardrail
...
| `src/decision/AGENTS.md` | Bounded probabilistic decisions (Jev System One) — contracts, engine registry, config, claim verification, context relevance, model tier |
